...
"""
Dispatch a dynamic injection against the complete network voltage/current arrays.

The default implementation preserves the traditional single-bus DynamicInjection
contract. External model packages can specialize this hook for a device whose
terminal set spans more than one bus without duplicating its dynamic states.
"""
function network_device!(
    device_states,
    output_ode,
    voltage_r,
    voltage_i,
