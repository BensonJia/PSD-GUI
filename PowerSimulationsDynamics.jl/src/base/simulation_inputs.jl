struct SimulationInputs
    ...
    function SimulationInputs(
        ...
        DAE_vector,
        mass_matrix,
        global_vars_update_pointers,
        global_state_map,
        global_inner_var_map,
        delays,
    )
        new(
            dynamic_injectors,
...
function _wrap_dynamic_injector_data(sys::PSY.System, lookup, injection_start::Int)
    ...
    for (ix, device) in enumerate(injector_data)
        ...
        configure_dynamic_device!(dynamic_device, lookup)
...
"""Optional hook for dynamic models that need the complete bus lookup."""
configure_dynamic_device!(::PSY.DynamicInjection, lookup) = nothing
