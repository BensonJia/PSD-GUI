...
function JacobianFunctionWrapper(
    ...
    if !isnothing(jacobian_prototype)
        size(jacobian_prototype) == (n, n) ||
            throw(IS.ConflictingInputsError("Cached Jacobian size does not match the model"))
        Jv = copy(jacobian_prototype)
