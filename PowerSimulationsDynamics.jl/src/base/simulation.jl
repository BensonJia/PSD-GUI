...
- `compiled_network` : A [`CompiledNetwork`](@ref) or exported project path to bind instead of rebuilding the network graph.
- `network_cache` : A cache root. The graph is loaded from or saved to a fingerprinted subdirectory automatically.
- `export_network` : A directory path (or `true` for `simulation_folder/network`) used to export topology, component equations, and the compiled graph after a successful build.
"""
function Simulation!(
    ::Type{T},
    system::PSY.System,
...
function _build_inputs!(
    sim::Simulation{T},
    compiled::Union{Nothing, CompiledNetwork} = nothing,
) where {T <: SimulationModel}
    simulation_system = get_system(sim)
    sim.inputs = if isnothing(compiled)
        SimulationInputs(T, simulation_system, sim.frequency_reference)
    else
        _bind_compiled_inputs(T, simulation_system, sim.frequency_reference, compiled)
    end
...
function _get_jacobian(
    ...
    return JacobianFunctionWrapper(
        ...
        jacobian_prototype =
            isnothing(compiled) ? nothing : compiled.jacobian_prototype,
    )
end
...
function _compiled_network_for_build(sim::Simulation{T}, kwargs) where {T <: SimulationModel}
    explicit = _resolve_compiled_network(get(kwargs, :compiled_network, nothing))
    if !isnothing(explicit)
        _validate_compiled_structure(T, get_system(sim), sim.frequency_reference, explicit)
        return explicit
    end
    cache_root = get(kwargs, :network_cache, nothing)
    isnothing(cache_root) && return nothing
    cache_root isa AbstractString ||
        throw(ArgumentError("network_cache must be a directory path or nothing"))
    fingerprint = network_fingerprint(
        T,
        get_system(sim);
        frequency_reference = sim.frequency_reference,
    )
    directory = _network_cache_directory(cache_root, fingerprint)
    file = _compiled_file(directory)
    isfile(file) || return nothing
    try
        compiled = load_compiled_network(directory)
        _validate_compiled_structure(T, get_system(sim), sim.frequency_reference, compiled)
        @info "Using compiled network graph $fingerprint"
        return compiled
    catch e
        bt = catch_backtrace()
        @warn "Ignoring incompatible compiled network cache" directory exception = (e, bt)
        return nothing
    end
end

function _save_network_after_build!(
    sim::Simulation{T},
    kwargs,
    loaded::Union{Nothing, CompiledNetwork},
) where {T <: SimulationModel}
    sim.status == BUILT || return
    compiled = nothing
    cache_root = get(kwargs, :network_cache, nothing)
    if !isnothing(cache_root) && isnothing(loaded)
        cache_root isa AbstractString ||
            throw(ArgumentError("network_cache must be a directory path or nothing"))
        try
            compiled = compile_network(sim)
            directory = _network_cache_directory(cache_root, compiled.fingerprint)
            save_compiled_network(compiled, directory)
            @info "Saved compiled network graph" directory
        catch e
            bt = catch_backtrace()
            @warn "Unable to save compiled network cache" cache_root exception = (e, bt)
        end
    end
    export_path = get(kwargs, :export_network, nothing)
    if !isnothing(export_path) && export_path !== false
        directory = export_path === true ? joinpath(sim.simulation_folder, "network") : export_path
        directory isa AbstractString ||
            throw(ArgumentError("export_network must be a directory path, true, false, or nothing"))
        export_network(sim, directory)
        @info "Exported network project" directory
    end
    return
end
...
function _build!(sim::Simulation{T}; kwargs...) where {T <: SimulationModel}
    ...
    TimerOutputs.@timeit BUILD_TIMER "Build Simulation" begin
        try
            ...
            compiled = _compiled_network_for_build(sim, kwargs)
            TimerOutputs.@timeit BUILD_TIMER "Build Simulation Inputs" begin
                _build_inputs!(sim, compiled)
                sim.multimachine =
            ...
            if sim.status != BUILD_FAILED
                ...
                TimerOutputs.@timeit BUILD_TIMER "Calculate Jacobian" begin
                    jacobian = _get_jacobian(sim, compiled)
                end
                ...
                TimerOutputs.@timeit BUILD_TIMER "Save Compiled Network" begin
                    _save_network_after_build!(sim, kwargs, compiled)
                end
